...
const RegisterForm = () => {
  ...
  function handleFormSubmit(event) {
    ...
    registerUser().then((result) => {
      if (result.status === "Success") {
        ...
        toast(
          "You will be notified on updates when hackathon is about to start",
          { duration: 3000 }
        );
      } else {
        toast.error("Failed to register");
        toast("Please retry again", { duration: 3000 });
      }
      setRegState(false);
    });
  }
  ...
  return (
    ...
    >
      ...
      <Spacer />
      <Input
        onChange={(e) => handleTextChange(e, setAge)}
        value={age}
        clearable
        placeholder="Age"
        htmlType="number"
      ...
      <Spacer />
      <Input
        onChange={(e) => handleTextChange(e, setSchool)}
        value={school}
        clearable
